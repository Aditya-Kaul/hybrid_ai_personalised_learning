...
def get_student_progress(student_modules):
    total_modules = len(student_modules)
    completed_modules = sum(1 for module in student_modules if module['status'] == 1)
    current_module = next((module for module in student_modules if module['status'] == 0), None)
    return total_modules, completed_modules, current_module

def display_student_profile(student):
    st.subheader(f"Student Profile: {student['name']}")
    st.write(f"Email: {student['email']}")
    
    modules = json.loads(student['modules'])
    total_modules, completed_modules, current_module = get_student_progress(modules)
    ...
    if current_module:
        st.subheader("Current Module")
        st.write(current_module['module_name'])
        st.progress(current_module['progress'])
        st.write(f"Module Progress: {current_module['progress']*100:.2f}%")
...
def app():
    ...
    selected_student_email = st.selectbox("Select a student", student_emails)
    
    selected_student = next((student for student in students if student['email'] == selected_student_email), None)
    
    if selected_student:
        display_student_profile(selected_student)
        
        st.subheader("Exercises Submitted")
        modules = json.loads(selected_student['modules'])
        for module in modules:
            if module['status'] == 1:  
                # ex = get_module_exercise(module_name=module['module_name'])
                ex = next((lesson for lesson in module['lessons'] if lesson["name"] == 'Exercises'), None)
                results = ex['results']
                with st.expander(module['module_name']):
                    st.write(f"Submission date: {random.randint(1, 28)}/{random.randint(1, 12)}/2024")
                    st.write(f"Score: {random.randint(70, 100)}%")
                    ...
                    st.subheader("Questions and Answers")
                    if results:
                        for idx, (question, answer) in enumerate(results.items()):
                            st.write(f"Q{idx+1}: {question}")
                            st.text_area(f"Answer {idx+1}", value=answer, key=f"{module['module_name']}_{idx}", disabled=True)
